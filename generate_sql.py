...
def get_schema(db_id, tables_file='tables.json'):
    ...
    for db in tables:
        if db['db_id'] == db_id:
            schema = []
            for table, columns in zip(db['table_names'], db['column_names']):
                schema.append(f"Table: {table}, Columns: {', '.join([col[1] for col in columns if col[0] != -1])}")
            return '\n'.join(schema)
    return ""
...
with open('predicted_sql.txt', 'w') as f:
    for i, item in enumerate(dev_data):
        ...
        prompt = f"""Given the following database schema:{schema}
                     Translate this natural language question into a valid SQL query:{question}
